...
@Component({
          ...
          <div class="px-8 pt-6 pb-6 space-y-4">
            <div class="bg-white dark:bg-slate-900 rounded-xl border border-primary/10 shadow-sm p-4">
              <div class="relative">
                ...
                <input [(ngModel)]="searchTerm" type="text" placeholder="Search artifacts, versions, hardware part numbers..."
              ...
              <div class="flex items-center gap-1 bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
                @for (p of platforms; track p.id) {
                  <button (click)="activePlatform = p.id"
                    [class]="'flex items-center gap-2 px-3 md:px-4 py-1.5 md:py-2 rounded-lg text-[10px] md:text-xs font-bold transition-all whitespace-nowrap ' + (activePlatform === p.id ? 'bg-white dark:bg-slate-700 text-primary shadow-sm' : 'text-slate-500 hover:bg-white/50')">
            ...
            <table class="w-full text-left border-collapse min-w-max">
              ...
              <tbody class="divide-y divide-slate-100 dark:divide-slate-800/60">
                @for (app of filteredArtifacts; track app.id) {
                  <tr (click)="onAppVersionContextMenu($event, app)" [class]="'group hover:bg-primary/5 transition-all cursor-pointer ' + (app.isDotCancelled ? 'bg-red-50/10' : '')">
                    ...
                    <td class="px-5 py-4">
                      <div class="inline-flex flex-col gap-1 p-2 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 cursor-context-menu hover:bg-primary/5 transition-all w-full">
                        ...
                        @if (app.store_upload_status && app.store_upload_status !== 'none') {
                          <div class="flex items-center gap-2 mt-2 pt-2 border-t border-slate-100 dark:border-slate-800">
                            <span [class]="'px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider cursor-help ' + getStoreStatusBadge(app.store_upload_status)" [title]="app.store_upload_message || 'No details available'">
                              Store: {{ app.store_upload_status }}
                            </span>
                            @if (app.fastlane_log_path) {
                              <button type="button" (click)="$event.stopPropagation(); viewLog(app.id)" class="text-[9px] font-bold text-blue-500 hover:underline">VIEW LOGS</button>
                            }
                          </div>
                        }
                      </div>
                    ...
                    <td class="px-5 py-4">
                      <span [class]="'px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ' + getStatusBadge(app.status)">{{ app.status }}</span>
                    </td>
                    <td class="px-5 py-4 text-center">
                      <div class="flex items-center justify-center gap-2">
                        <a [href]="app.userManualUrl" target="_blank" (click)="$event.stopPropagation()" class="p-1.5 hover:bg-red-50 text-red-500 rounded-lg inline-flex" title="View Manual">
                          <span class="material-symbols-outlined text-[20px]">picture_as_pdf</span>
                        </a>
                        <button type="button" (click)="$event.stopPropagation(); deleteApp(app.id)" class="p-1.5 hover:bg-red-600 hover:text-white text-red-500 rounded-lg inline-flex transition-all" title="Delete Permanent">
                          <span class="material-symbols-outlined text-[20px]">delete</span>
                        </button>
                      </div>
                    </td>
                    <td class="px-5 py-4 text-xs font-medium text-slate-600 dark:text-slate-400 whitespace-nowrap">{{ app.effectiveDate }}</td>
                    <td class="px-5 py-4">
                      <div class="flex items-center gap-2">
                        @if (app.triggerLoginUpdate) { <span class="px-2 py-0.5 rounded-md bg-blue-50 text-blue-600 text-[9px] font-bold uppercase tracking-wider">OTA</span> }
                        @if (app.isDotCancelled) { <span class="px-2 py-0.5 rounded-md bg-red-50 text-red-600 text-[9px] font-bold uppercase tracking-wider">DOT</span> }
                      </div>
                    </td>
                  </tr>
                }
  ...
  @if (showContextMenu) {
    <div class="fixed z-[100] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-2xl p-1.5 min-w-[200px]" [style.left.px]="menuX" [style.top.px]="menuY">
      ...
      <button type="button" (click)="downloadArtifact(selectedApp)" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold hover:bg-slate-100">
        <span class="material-symbols-outlined text-[18px]">download</span> Download Binary
      </button>
      <button type="button" (click)="openVersionHistoryFor(selectedApp)" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold hover:bg-slate-100">
        <span class="material-symbols-outlined text-[18px]">history</span> Version History
      </button>
      @if (selectedApp?.platform === 'Android') {
        <button type="button" (click)="triggerStoreUpload(selectedApp)" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold text-blue-600 hover:bg-blue-50 mt-1">
      ...
      @if (selectedApp?.platform === 'Android' || selectedApp?.platform === 'iOS') {
        <button type="button" (click)="selectedApp && loadLiveStoreVersion(selectedApp)" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold text-emerald-600 hover:bg-emerald-50 mt-1">
      ...
      <button type="button" (click)="selectedApp && moveToArchive(selectedApp.id)" class="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-xs font-semibold text-slate-600 hover:bg-slate-50 mt-1">
    ...
    <div class="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" (click)="closeUploadModal()">
      <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto" (click)="$event.stopPropagation()">
        <div class="px-8 py-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <div>
            <h2 class="text-lg font-bold text-slate-900 dark:text-white">{{ editingAppId ? 'Edit Artifact' : 'New Artifact Upload' }}</h2>
            <p class="text-[10px] text-slate-500 uppercase tracking-widest mt-0.5">All files are encrypted and locked after upload.</p>
          </div>
          <button type="button" (click)="closeUploadModal()" class="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
        ...
        <form [formGroup]="uploadForm" (ngSubmit)="submitUpload()" class="px-8 py-6 space-y-6">
          ...
          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-1.5">
              ...
              <input formControlName="description" placeholder="e.g. ELD Application" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
            <div class="space-y-1.5">
          ...
          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-1.5">
              ...
              <input formControlName="appVersion" placeholder="e.g. ABC-235000-1.01" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
            <div class="space-y-1.5">
              <label class="block text-[10px] font-bold text-slate-500 uppercase tracking-wider">OS VERSION *</label>
              <input formControlName="osVersion" placeholder="e.g. Android 12.1" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
          </div>
          ...
          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-1.5">
              ...
              <input formControlName="packageName" placeholder="e.g. com.gbt.eld" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 outline-none focus:ring-2 focus:ring-primary/20 transition-all" />
            </div>
            <div class="space-y-1.5">
          ...
          <div class="grid grid-cols-2 gap-6">
            <div class="space-y-1.5">
              ...
              <input type="file" accept=".pdf,.doc,.docx" (change)="onManualSelected($event)" class="hidden" #manualInput />
              <button type="button" (click)="manualInput.click()" class="w-full px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50 text-left text-sm text-slate-500 flex items-center gap-2">
          ...
          <div class="flex items-center justify-between bg-blue-50 dark:bg-blue-900/20 p-4 rounded-2xl border border-blue-100 dark:border-blue-800 group">
            <div class="flex items-center gap-2">
            ...
            <div class="relative flex items-center justify-center">
              <span class="material-symbols-outlined text-blue-500 cursor-help hover:text-blue-700 transition-colors">info</span>
              
              <!-- Popover content -->
              <div class="absolute bottom-full right-0 mb-2 w-[340px] md:w-[400px] bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl rounded-xl p-4 opacity-0 pointer-events-none group-hover:opacity-100 group-hover:pointer-events-auto transition-all z-50">
                <h4 class="font-bold text-sm text-slate-800 dark:text-white mb-3 flex items-center gap-1.5">
                  <span class="material-symbols-outlined text-[18px] text-blue-500">settings_applications</span> 
                  Fastlane Requirements
                </h4>
                
                <div class="space-y-3">
                  <div class="bg-emerald-50 dark:bg-emerald-900/20 p-3 rounded-lg border border-emerald-100 dark:border-emerald-800">
                    <p class="text-xs font-bold text-emerald-800 dark:text-emerald-300 flex items-center gap-1.5 mb-1.5">
                      <span class="material-symbols-outlined text-[16px]">android</span> Android (Google Play)
                    </p>
                    <ul class="text-[10px] md:text-xs text-emerald-700 dark:text-emerald-400 space-y-1 list-disc pl-4 font-medium">
                      <li>Must upload an <b>.aab</b> or <b>.apk</b> file.</li>
                      <li><b>Package Name</b> must exactly match the Play Store identifier.</li>
                      <li>Requires Google Service Account JSON key on the server.</li>
                    </ul>
                  </div>
                  
                  <div class="bg-slate-100 dark:bg-slate-700/50 p-3 rounded-lg border border-slate-200 dark:border-slate-600">
                    <p class="text-xs font-bold text-slate-800 dark:text-slate-200 flex items-center gap-1.5 mb-1.5">
                      <span class="material-symbols-outlined text-[16px]">apple</span> iOS (App Store Connect)
                    </p>
                    <ul class="text-[10px] md:text-xs text-slate-600 dark:text-slate-400 space-y-1 list-disc pl-4 font-medium">
                      <li>Must upload an <b>.ipa</b> file.</li>
                      <li><b>Bundle ID</b> (Package Name field) must exactly match App Store Connect.</li>
                      <li>Requires App Store Connect API Key (AuthKey.p8) on the server.</li>
                    </ul>
                  </div>
                </div>
                
                <!-- Triangle pointer -->
                <div class="absolute -bottom-1.5 right-1 w-3 h-3 bg-white dark:bg-slate-800 border-b border-r border-slate-200 dark:border-slate-700 transform rotate-45"></div>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-1.5">
            ...
            <div class="bg-slate-50/50 dark:bg-slate-800/50 p-6 rounded-2xl border border-dashed border-slate-300 dark:border-slate-700 group hover:border-primary/50 transition-all">
              <input type="file" #fileInput class="hidden" accept=".apk,.aab,.ipa,.exe,.bin,.zip" (change)="onFileSelected($event)" />
              <button type="button" (click)="fileInput.click()" class="w-full flex flex-col items-center gap-2">
                ...
                <div class="text-center">
                  <p class="text-sm font-bold text-slate-700 dark:text-slate-200">{{ uploadedFileName || 'Click to upload .apk, .aab, .ipa, .exe, .zip, or binary' }}</p>
          ...
          <div class="flex items-center gap-4 pt-4">
            <button type="submit" [disabled]="uploadForm.invalid" class="flex-1 bg-primary text-white py-3.5 rounded-2xl font-bold text-sm shadow-xl shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95 disabled:opacity-50 disabled:active:scale-100">
              Save Artifact
            </button>
          </div>
        </form>
    ...
    <div class="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4" (click)="showVersionHistoryModal = false">
      <div class="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden" (click)="$event.stopPropagation()">
        <div class="px-6 py-4 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
          ...
          <button type="button" (click)="showVersionHistoryModal = false" class="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
            <span class="material-symbols-outlined text-slate-400">close</span>
          </button>
        </div>
...
export class AppsComponent implements OnInit {
  ...
  loadArtifacts() {
    this.http.get<any[]>(`${API_BASE_URL}api/apps.php?cmd=get_all_apps&t=${Date.now()}`).subscribe({
      next: (data) => {
        ...
        const isAnyProcessing = this.artifacts.some(a => a.store_upload_status === 'processing' || a.store_upload_status === 'pending');
        if (isAnyProcessing) {
  ...
  loadLiveStoreVersion(app: AppArtifact) {
    ...
    this.http.get<any>(`${API_BASE_URL}api/apps.php?cmd=check_live_version&company=${encodeURIComponent(app.company)}`).subscribe({
      next: (res) => {
        if ((res.success === true || res.status === 'success') && res.live_version) {
  ...
  mapBackendToArtifact(data: any): AppArtifact {
    return {
      ...
      effectiveDate: data.created_at ? String(data.created_at).split(' ')[0] : 'N/A',
      fwEffectiveDate: data.created_at ? String(data.created_at).split(' ')[0] : 'N/A',
      status: (data.artifact_status || 'Stable') as ArtifactStatus,
      type: (data.app_type || 'ELD') as AppType,
      uploadedAt: data.created_at || '',
      isLocked: data.is_locked == 1 || data.is_locked === true,
      isEncrypted: data.is_encrypted == 1 || data.is_encrypted === true,
      isDotCancelled: data.dot_cancelled == 1 || data.dot_cancelled === true,
      triggerLoginUpdate: data.trigger_login_update == 1 || data.trigger_login_update === true,
      userManualUrl: data.user_manual ? `${API_BASE_URL}/storage/apps/${data.user_manual}` : '#',
      package_name: data.package_name || '',
      release_track: data.release_track || 'internal',
      auto_upload: data.auto_upload == 1 || data.auto_upload === true,
  ...
  getStoreStatusBadge(status: string) {
    if (status === 'success') return 'bg-emerald-50 text-emerald-600';
    if (status === 'processing' || status === 'pending') return 'bg-amber-50 text-amber-600 animate-pulse';
  ...
  onFileSelected(e: Event) {
    ...
    const allowedExtensions = ['apk', 'aab', 'ipa', 'exe', 'bin', 'zip'];
    const fileExtension = file.name.split('.').pop()?.toLowerCase();

    if (!fileExtension || !allowedExtensions.includes(fileExtension)) {
      alert('Invalid file type. Please upload .apk, .aab, .ipa, .exe, .bin, or .zip file.');
      input.value = '';
      this.uploadedFileName = '';
      this.binaryFile = null;
      return;
  ...
  submitUpload() {
    ...
    this.http.post<any>(`${API_BASE_URL}api/apps.php`, formData).subscribe({
      next: (resp) => {
        alert(resp.message || 'Artifact saved successfully.');
        this.loadArtifacts();
        this.closeUploadModal();

        if (v.autoUpload && resp.id && (v.platform === 'Android' || v.platform === 'iOS')) {
  ...
  editApp(app: AppArtifact) {
    ...
    this.uploadForm.patchValue({
      ...
      status: app.status,
      isDotCancelled: app.isDotCancelled,
      packageName: app.package_name || '',
      releaseTrack: app.release_track || 'internal',
      autoUpload: app.auto_upload == true
    });
  ...
  moveToArchive(id: number) {
    if (!confirm('Move this record to Archive? It will disappear from main Repository.')) return;
  ...
  downloadArtifact(app: AppArtifact | null) {
    if (!app) return;
    if (app.binary_file_name) {
      window.open(`${API_BASE_URL}storage/apps/${app.binary_file_name}`, '_blank');
    } else {
      alert('No binary available.');
    }
    this.showContextMenu = false;
  }
    ...
    this.http.post<any[]>(`${API_BASE_URL}api/apps.php`, formData).subscribe({
      next: (data) => {
        this.versionHistory = (data || []).map(i => ({
          version: i.app_version,
          date: i.created_at ? String(i.created_at).split(' ')[0] : 'N/A',
          platform: i.platform as Platform,
          status: i.artifact_status as ArtifactStatus
        }));
  ...
  triggerStoreUpload(app: AppArtifact | null) {
    ...
    if (!isAndroid && !isIOS) {
      alert('Store upload is only available for Android and iOS apps.');
      this.showContextMenu = false;
      return;
    }

    const storeName = isAndroid ? 'Google Play Store' : 'Apple Store';
    const actionDesc = `upload version ${app.appVersion} to the ${storeName}?`;
  ...
  handleStoreUploadTrigger(id: number, platform: string, version: string) {
    this.http.get<any>(`${API_BASE_URL}api/apps.php?cmd=store_upload&id=${id}`).subscribe({
      next: (resp) => {
        alert(resp.message || 'Store upload started.');
        this.loadArtifacts();
      },
      error: (err) => {
